...
(function($) {
    ...
    ErrorLog.prototype.requestNotifications = function() {
        notify.requestPermission();
    };

    ErrorLog.prototype.sendNotification = function(title, body, closeAfter) {
        var notification = notify.createNotification(title, {
            body: body,
            icon: window.location+'Error.ico'
        });
        if (closeAfter) {
            setTimeout(function() { notification.close(); }, closeAfter);
        }
    };
